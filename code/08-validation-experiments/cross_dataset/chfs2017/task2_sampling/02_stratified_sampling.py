#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
02_stratified_sampling.py
对 task1 筛选出的灵活就业人员按参保险种分层随机抽样，共抽取 500 个样本。

分层规则（基于 f1001a + f1008_imp 推断2016年参保状态）：
  城镇职工保：f1001a == 2  且  NOT (f1008_imp == 0)
  城乡居民保：f1001a ∈ {3,4,5}  且  NOT (f1008_imp == 0)
  不参保    ：f1001a == 7788  OR  f1008_imp == 0  OR  f1001a == 7777

  说明：f1008_imp 为 NaN 时不判为不参保（问卷可能未问到缴费金额，
        但 f1001a 有效值本身即表示参保）

可复现性：random_state=42

输出:
  task2_sampling/sampled_500.csv
  task2_sampling/02_sampling_log.txt
"""

from pathlib import Path
import numpy as np
import pandas as pd

# Paths
SCRIPT_DIR = Path(__file__).resolve().parent
IN_CSV     = SCRIPT_DIR.parent / "task1" / "filtered_flexible_workers.csv"
OUT_CSV    = SCRIPT_DIR / "sampled_500.csv"
LOG_PATH   = SCRIPT_DIR / "02_sampling_log.txt"

TOTAL_N     = 500
RANDOM_SEED = 42

_log = open(LOG_PATH, "w", encoding="utf-8")

def log(msg=""):
    print(msg)
    _log.write(msg + "\n")
    _log.flush()

log("=" * 70)
log("Task 2: CHFS 2017 stratified random sample (N=500)")
log("=" * 70)

# Load filtered data
log(f"\n[1] Loading {IN_CSV.name}...")
df = pd.read_csv(IN_CSV, dtype={"hhid": str, "pline": str})
log(f"  Input sample size: {len(df):,}")

# Assign pension-scheme strata
log("\n[2] Assigning strata from f1001a and f1008_imp...")

def classify_stratum(row):
    """
    优先级：城镇职工保 > 城乡居民保 > 不参保
    f1008_imp == 0  → 本年度未缴费 → 按不参保处理
    f1008_imp NaN   → 未询问/缺失 → 不强制判为不参保（以 f1001a 为准）
    """
    f1001a    = row["f1001a"]
    f1008_imp = row["f1008_imp"]

    # Explicit non-participation.
    if f1001a == 7788:
        return "不参保"
    if pd.notna(f1008_imp) and f1008_imp == 0:
        return "不参保"
    if f1001a == 7777:          # Treat unclassified schemes as non-participation.
        return "不参保"

    # Employee scheme.
    if f1001a == 2:
        return "城镇职工保"

    # Resident scheme, including rural, urban-resident, and unified variants.
    if f1001a in [3, 4, 5]:
        return "城乡居民保"

    return "不参保"

df["stratum"] = df.apply(classify_stratum, axis=1)

stratum_order = ["城镇职工保", "城乡居民保", "不参保"]
counts = df["stratum"].value_counts()
total  = len(df)

# Proportional allocation
raw_alloc = {s: counts.get(s, 0) / total * TOTAL_N for s in stratum_order}
alloc     = {s: int(round(v)) for s, v in raw_alloc.items()}

diff = TOTAL_N - sum(alloc.values())
if diff != 0:
    remainders = sorted(
        stratum_order,
        key=lambda s: raw_alloc[s] - int(raw_alloc[s]),
        reverse=(diff > 0),
    )
    for i in range(abs(diff)):
        alloc[remainders[i]] += 1 if diff > 0 else -1

log(f"\n  {'Stratum':<10} {'Total':>8} {'Share':>7} {'Sample':>7}")
log(f"  {'─'*40}")
for s in stratum_order:
    n = counts.get(s, 0)
    log(f"  {s:<10} {n:>8,}  {n/total*100:>6.1f}%  {alloc[s]:>7}")
log(f"  {'─'*40}")
log(f"  {'Total':<10} {total:>8,}  {'100.0%':>7}  {sum(alloc.values()):>7}")

# Draw stratified sample
log(f"\n[3] Drawing the stratified random sample (seed={RANDOM_SEED})...")

sampled_parts = []
for s in stratum_order:
    sub  = df[df["stratum"] == s]
    n    = alloc[s]
    samp = sub.sample(n=n, random_state=RANDOM_SEED)
    sampled_parts.append(samp)

sampled_df = pd.concat(sampled_parts, ignore_index=True)
assert len(sampled_df) == TOTAL_N

log("\n  Sample validation:")
for s in stratum_order:
    n = (sampled_df["stratum"] == s).sum()
    log(f"    {s}: {n} records")
log(f"    Total: {len(sampled_df)} records (seed={RANDOM_SEED})")

# Save artifacts
log("\n[4] Saving...")
sampled_df.to_csv(OUT_CSV, index=False, encoding="utf-8-sig")
log(f"  Saved: {OUT_CSV}")
log(f"  Rows: {len(sampled_df)}; columns: {len(sampled_df.columns)}")
log("\nComplete")
_log.close()
