...
# -*- coding: utf-8 -*-
"""
04_build_prompts_v2.py
Build prompts and ground-truth records for the 500 CHFS 2017 samples
reconstructed in task3-v2. The input is reconstructed_500_v2.csv, including
estimated cumulative contribution years.

Use the new-era prompt template unchanged from
test_all_samples/generate_prompts_newera.py. Province-level 2016 pension policy
data comes from province_policy_2016.xlsx.

Outputs:
  data/prompts/generalization/chfs2017/prompts_2017_v2.json
  data/prompts/generalization/chfs2017/ground_truth_2017_v2.json
"""
...
TASK3_CSV   = SCRIPT_DIR.parent / "task3_reconstructing" / "reconstructed_500_v2.csv"
PROJECT_ROOT = Path(__file__).resolve().parents[5]
POLICY_XLSX = PROJECT_ROOT / "data" / "raw" / "policy" / "province_policy_2016.xlsx"
OUTPUT_DIR = PROJECT_ROOT / "data" / "prompts" / "generalization" / "chfs2017"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_PROMPTS = OUTPUT_DIR / "prompts_2017_v2.json"
OUT_GT      = OUTPUT_DIR / "ground_truth_2017_v2.json"

print("=" * 70)
print("Task 4: Build Prompts and Ground Truth (CHFS 2017)")
print("=" * 70)
...
wb         = openpyxl.load_workbook(POLICY_XLSX, data_only=True)
ws         = wb["总表"]
...
for idx, row in policy_df.iterrows():
    ...
    # Annual contribution-base lower bound.
    ...
    # Annual contribution-base upper bound.
    ...
    # Annual contribution lower bound.
    ...
    # Annual contribution upper bound.
...
def match_policy(province: str):
    if not province or province in ("不清楚", "nan", ""):
        return None
    ...
    if len(matched) > 0:
        p = matched.iloc[0]
        return _extract_policy(p)
    
    # Abbreviation mapping.
    for abbr, full in PROVINCE_ABBR.items():
        if abbr in province:
            matched = policy_df[policy_df["省名"] == full]
            if len(matched) > 0:
                p = matched.iloc[0]
                return _extract_policy(p)
    
    # Match after removing administrative suffixes.
    for suffix in ["省","市","自治区","壮族自治区","回族自治区","维吾尔自治区","藏族自治区"]:
        stripped = province.replace(suffix, "")
        if stripped in PROVINCE_ABBR:
            matched = policy_df[policy_df["省名"] == PROVINCE_ABBR[stripped]]
            if len(matched) > 0:
                p = matched.iloc[0]
                return _extract_policy(p)
    # Partial-name match.
        for _, row in policy_df.iterrows():
            ...
            if full_name.replace(suffix, "") == stripped:
                return _extract_policy(row)
    
    return None

def _extract_policy(p):
    """Extract policy fields from a matched row."""
    def safe_int(col):
        v = p.get(col)
        try:   return str(int(float(v))) if pd.notna(v) else "不清楚"
        except: return "不清楚"

    def fmt_index(col):
        v = p.get(col)
        try:   return f"{float(v):g}" if pd.notna(v) else "不清楚"
        except: return "不清楚"
...
# 4. Chinese new-era prompt template
PROMPT_TEMPLATE = """# 角色
...
def na_if_unclear(val: str) -> str:
    return "N/A" if str(val).strip() in ("不清楚", "nan", "") else str(val)
...
for _, row in df.iterrows():
    ...
    if is_unclear:
        ...
        if proxy is None:
            print(f"  ✗ {sid}: hukou province is unknown and region ({region}) has no proxy province; skipping")
            skip_count += 1
            continue
        policy_vars = match_policy(proxy)
        if policy_vars is None:
            print(f"  ✗ {sid}: no policy match for proxy province ({proxy}); skipping")
            skip_count += 1
            continue
        proxy_count += 1
        ...
        if policy_vars is None:
            print(f"  ✗ {sid}: no policy match for province ({province_raw}); skipping")
            skip_count += 1
            continue
        policy_block = NORMAL_POLICY_BLOCK.format(**policy_vars)
    ...
    try:
        ...
        print(f"  ✗ {sid}: prompt template substitution failed: {e}; skipping")
        skip_count += 1
        continue
    ...
    # Ground truth action and account for the 2016/2017 decision window.
...
print(f"  Generated: {len(prompts)} | Skipped: {skip_count} | Proxy provinces used: {proxy_count}")
...
print("\n[5] Prompt example (first record, first 800 characters)")
print("=" * 70)
if prompts:
    print(prompts[0]["prompt"][:800])
    print("...")
print("=" * 70)
print(f"Complete: generated {len(prompts)} prompts.")
