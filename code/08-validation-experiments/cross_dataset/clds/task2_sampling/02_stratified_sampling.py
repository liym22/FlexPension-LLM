...
分层规则（直接使用 task1 已计算的 ins_type）：
  城镇职工养老保险 → 层1
  城乡居民养老保险 → 层2
  不参保           → 层3

抽样策略：等比分层（与 CFPS/CHIP 保持一致）。
...
log("=" * 70)
log("Task 2: CLDS 2018 stratified random sample (N=500)")
log("=" * 70)
...
# Allocate strata
log("\n[2] Assigning pension-channel strata...")
...
log(f"\n  {'Stratum':<14} {'Total':>8} {'Share':>7} {'Sample':>7}")
log(f"  {'─'*44}")
for s in stratum_order:
    n = counts.get(s, 0)
    log(f"  {s:<14} {n:>8,}  {n/total*100:>6.1f}%  {alloc[s]:>7}")
log(f"  {'─'*44}")
log(f"  {'Total':<14} {total:>8,}  {'100.0%':>7}  {sum(alloc.values()):>7}")
