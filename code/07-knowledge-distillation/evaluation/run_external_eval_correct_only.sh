...
# Correct-only ablation: run inference and evaluation on four external surveys.
# Resume support skips subtasks that already contain the expected number of valid results.
#
# Usage:
#   bash run_external_eval_correct_only.sh
#   ADAPTERS_PATH=/path/to/checkpoint TEMPERATURE=0.0 bash run_external_eval_correct_only.sh

INFERENCE_DIR="${SCRIPT_DIR}/../inference"
EXTERNAL_DIR="${SERVER_EXTERNAL_DATASET_DIR}"
OUTPUT_ROOT="${SERVER_OUTPUT_DIR}/external_results_correct_only"

MODEL_PATH="${MODEL_PATH:-"${SERVER_MODEL_PATH}"}"
# Use the latest correct-only checkpoint by default.
ADAPTERS_PATH="${ADAPTERS_PATH:-"$(find "${SERVER_OUTPUT_DIR}/correct_only" -type d -name 'checkpoint-*' | sort -V | tail -1)"}"
...
echo "====== Starting correct-only external evaluation: ${#DATASETS[@]} subtasks ======"
