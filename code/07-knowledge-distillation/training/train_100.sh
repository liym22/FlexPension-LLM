...
mkdir -p "${LOG_DIR}"
LOG_FILE=${LOG_FILE:-"${LOG_DIR}/full_train_100.log"}

# 4. Start ms-swift training with the Transformers backend.
DATASET_PATH=${DATASET_PATH:-"${SERVER_DATASET_DIR}/train.jsonl"}
VAL_DATASET_PATH=${VAL_DATASET_PATH:-"${SERVER_DATASET_DIR}/val.jsonl"}
OUTPUT_DIR=${OUTPUT_DIR:-"${SERVER_OUTPUT_DIR}/v2-data100"}
