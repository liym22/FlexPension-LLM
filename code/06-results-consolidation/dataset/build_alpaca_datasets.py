...
from config.paths import (
    ...
    DISTILLATION_DATASETS_FULL_DIR,
...
def main():
    ...
    output_dir = DISTILLATION_DATASETS_FULL_DIR
    ...
    test_ids_path = dataset_dir / "test_ids.json"

    if not train_ids_path.exists() or not val_ids_path.exists() or not test_ids_path.exists():
        raise FileNotFoundError("Missing train/val/test ID files")
    ...
    for sample_id in sorted(train_ids | val_ids):
        ...
        if renew_item and renew_item.get("parse_success") and renew_item.get("success"):
            parsed_json = renew_item.get("parsed_json")

        if not parsed_json:
            claude_item = claude_map.get(sample_id)
            if claude_item and claude_item.get("parse_success") and claude_item.get("success"):
                predicted_action = claude_item.get("predicted_action")
                predicted_type = claude_item.get("predicted_insurance_type")
                if (
                    predicted_action == gt.get("decision")
                    and (
                        gt.get("decision") == "不参保"
                        or predicted_type == gt.get("type")
                    )
                ):
    ...
    print("Alpaca dataset files generated.")
